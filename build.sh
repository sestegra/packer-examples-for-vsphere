...
menu_option_1() {
  INPUT_PATH="$SCRIPT_PATH"/builds/bsd/freebsd/13/
  echo -e "\nCONFIRM: Build a FreeBSD 13 Template for VMware vSphere?"
  echo -e "\nContinue? (y/n)"
  read -r REPLY
  if [[ ! $REPLY =~ ^[Yy]$ ]]
  then
  ...
  ### Build a FreeBSD 13 Template for VMware vSphere. ###
  echo "Building a FreeBSD 13 Template for VMware vSphere..."
