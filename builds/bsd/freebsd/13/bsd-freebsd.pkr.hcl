/*
    DESCRIPTION:
    FreeBSD 13 template using the Packer Builder for VMware vSphere (vsphere-iso).
*/
...
locals {
  ...
  data_source_content = {
    "/installerconfig" = templatefile("${abspath(path.root)}/data/installerconfig.pkrtpl.hcl", {
...
source "vsphere-iso" "bsd-freebsd" {
  ...
  boot_command = [
    ...
    "<wait15s>",
    "<enter><wait>",
    "mdmfs -s 16m md /tmp<enter><wait>",
    "mkdir /tmp/packer<enter><wait>",
    # "disk" data source
    "mount_cd9660 /dev/iso9660/PACKER /tmp/packer<enter><wait5>",
    # "http" data source
    // "dhclient -l /tmp/dhclient.lease.vmx0 vmx0<enter><wait5>",
    // "fetch -o /tmp/packer/installerconfig http://{{ .HTTPIP }}:{{ .HTTPPort }}/installerconfig<enter><wait5>",
    "bsdinstall -D /tmp/install.log script /tmp/packer/installerconfig<enter>",
  ]
  // boot_command = [
  //   "<esc>",
  //   "e",
...
build {
  sources = ["source.vsphere-iso.bsd-freebsd"]
